// Government Admin Portal — GovAdminLoginPage.tsx
...
const GovAdminLoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await govAdminAPI.login({
        email: identifier,
        password,
      });
      ...
      if (responseData?.access && responseData?.refresh) {
        ...
        if (!['lga_admin', 'state_admin', 'sth_admin', 'stth_admin'].includes(userData?.role)) {
          setError('This portal is for government administrators only.');
          setIsLoading(false);
          return;
        }
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <Badge className="bg-teal-700 text-white mb-3">
              <Building2 className="w-3 h-3 mr-1" />
              Gov Admin
            </Badge>
            <h1 className="text-2xl font-bold text-white">Government Admin Portal</h1>
            <p className="text-gray-400 text-sm mt-1">
              Sign in to manage government healthcare facilities
            </p>
          </div>
          ...
          <Card className="shadow-xl border-0 bg-slate-800/50 border-slate-700">
            ...
            <CardContent className="space-y-5">
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <Button
                  type="submit"
                  className="w-full h-11 bg-teal-600 hover:bg-teal-500 transition-colors"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
          ...
          <div className="text-center mt-6">
            <p className="text-xs text-gray-500">
              AI-MSHM Government Admin Portal v1.0.0
            </p>
          </div>
        </motion.div>
      </div>
...
export default GovAdminLoginPage;
