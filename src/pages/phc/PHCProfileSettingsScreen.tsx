...
import {
  ...
  Plus, Edit, Check, LogOut, HelpCircle, ExternalLink,
...
export default function PHCProfileSettingsScreen() {
  ...
  // Misc
  const [webhookUrl] = useState('https://hospital-ehr.internal/hl7-webhook');
    ...
    async function load() {
      try {
        ...
        if (notifData && typeof notifData === 'object') {
          setNotifications({
            new_referral: !!(notifData.risk_score_updates_enabled ?? notifData.new_referral),
            score_change: !!(notifData.risk_score_updates_enabled ?? notifData.score_change),
            overdue_followup: !!(notifData.evening_checkin_enabled ?? notifData.overdue_followup),
            missed_checkin: !!(notifData.morning_checkin_enabled ?? notifData.missed_checkin),
          });
        }
      } catch {
        // silent
  ...
  // ── Load staff when staff tab is opened (lazy) ────────────────────────────
  useEffect(() => {
    if (activeTab === 'staff' && isAdmin && staff.length === 0) {
      setStaffLoading(true);
      apiClient.get('/centers/phc/staff/')
        .then((res) => {
          const sData = res.data?.data ?? res.data ?? [];
          setStaff(Array.isArray(sData) ? sData : [sData]);
        })
        .catch(() => {})
        .finally(() => setStaffLoading(false));
    }
  }, [activeTab, isAdmin, staff.length]);
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      setPasswordError(err.response?.data?.detail || 'Network error');
    }
  };
  ...
  const handleNotificationToggle = async (key: keyof NotificationSettings) => {
    ...
    try {
      const res = await apiClient.patch('/settings/notifications/', {
        risk_score_updates_enabled: updated.new_referral,
        morning_checkin_enabled: updated.missed_checkin,
        evening_checkin_enabled: updated.overdue_followup,
      });
  ...
  const handleLogout = async () => {
    try { await apiClient.post('/auth/logout/', { refresh: localStorage.getItem('refresh_token') }); } catch { }
  ...
  const getRoleBadge = (role: string) => {
    const colors: Record<string, string> = { cho: 'bg-blue-100 text-blue-800', assistant: 'bg-amber-100 text-amber-800', receptionist: 'bg-purple-100 text-purple-800', other: 'bg-gray-100 text-gray-800' };
    const labels: Record<string, string> = { cho: 'CHO', assistant: 'Assistant', receptionist: 'Receptionist', other: 'Other' };
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    ...(isAdmin ? [{ id: 'staff' as Tab, label: 'Staff', icon: <Users className="w-4 h-4" /> }] : []),
  ];
    ...
    <PHCLayout>
          ...
          {activeTab === 'settings' && (
            ...
            >
              ...
              <Card className="border-l-4 border-[#2E8B57]">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" className="border-[#2E8B57] text-[#2E8B57] hover:bg-green-50" onClick={() => setShowEditProfile(!showEditProfile)}>
                ...
                <CardContent>
                  {showEditProfile ? (
                    <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                      {[
                        { l: 'Facility Name', v: profile?.name },
                        { l: 'Code', v: profile?.code },
                        { l: 'State', v: profile?.state },
                        { l: 'LGA', v: profile?.lga },
                        { l: 'Phone', v: profile?.phone },
                        { l: 'Escalation FMC', v: profile?.escalation_fmc_name || 'Not set' },
                      ].map((f) => (
                        <div key={f.l}><Label>{f.l}</Label><Input value={f.v || ''} disabled /></div>
                      ))}
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="flex items-center gap-2"><Building className="w-4 h-4 text-gray-400" /><span className="text-gray-500">Name:</span><span className="font-medium">{profile?.name || 'N/A'}</span></div>
                      <div className="flex items-center gap-2"><span className="text-gray-500">Code:</span><span className="font-medium">{profile?.code || 'N/A'}</span></div>
                      <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-gray-400" /><span className="text-gray-500">State:</span><span className="font-medium">{profile?.state || 'N/A'}</span></div>
                      <div className="flex items-center gap-2"><span className="text-gray-500">LGA:</span><span className="font-medium">{profile?.lga || 'N/A'}</span></div>
                      <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-gray-400" /><span className="text-gray-500">Phone:</span><span className="font-medium">{profile?.phone || 'N/A'}</span></div>
                      <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-gray-400" /><span className="text-gray-500">Hours:</span><span className="font-medium">Mon–Fri 8am–5pm</span></div>
                      {profile?.escalation_fmc_name && (
                        <div className="col-span-2 flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-gray-400" /><span className="text-gray-500">Escalation FMC:</span><span className="font-medium">{profile.escalation_fmc_name}</span></div>
                      )}
                    </div>
                  )}
                </CardContent>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-[#2E8B57] flex items-center justify-center text-white font-bold text-lg">
                      {user?.full_name?.charAt(0) || 'U'}
                    </div>
                    <div>
                      <p className="font-semibold text-lg">{user?.full_name || 'PHC Staff'}</p>
                      {user?.unique_id && <p className="text-sm text-[#2E8B57] font-medium">{user.unique_id}</p>}
                  ...
                  <Button variant="link" className="text-[#2E8B57] p-0 h-auto font-normal" onClick={() => setShowPasswordForm(!showPasswordForm)}>
                    Change Password
                  </Button>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t">
                    <div className="flex items-center gap-2 text-sm"><span>Critical escalation alerts</span><Lock className="w-3 h-3 text-gray-400" /></div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">FHIR Connection Status</span>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <a href="#" className="flex items-center gap-2 text-[#2E8B57] hover:underline text-sm"><ExternalLink className="w-4 h-4" /> Help Documentation</a>
                  <a href="mailto:support@ai-mshm.org" className="flex items-center gap-2 text-[#2E8B57] hover:underline text-sm"><Mail className="w-4 h-4" /> Contact AI-MSHM Support</a>
                </CardContent>
              </Card>
              ...
              <Card className="border-red-200">
                <CardContent className="pt-6">
                  <Button variant="outline" className="w-full border-red-300 text-red-600 hover:bg-red-50" onClick={() => setShowLogoutDialog(true)}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input value={newStaffForm.full_name} onChange={(e) => { setNewStaffForm({ ...newStaffForm, full_name: e.target.value }); setFormErrors((p) => ({ ...p, full_name: '' })); }} placeholder="Enter full name" className={formErrors.full_name ? 'border-red-500' : ''} />
            </div>
            <div>
            ...
            <div>
              <Label>Role</Label>
              <select value={newStaffForm.staff_role} onChange={(e) => setNewStaffForm({ ...newStaffForm, staff_role: e.target.value })} className="w-full h-10 px-3 border rounded-md text-sm mt-1">
            ...
            <div>
              ...
              <Input value={newStaffForm.employee_id} onChange={(e) => setNewStaffForm({ ...newStaffForm, employee_id: e.target.value })} placeholder="Enter employee ID" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAddStaffModal(false)}>Cancel</Button>
            <Button onClick={handleAddStaff} disabled={!newStaffForm.full_name || !newStaffForm.email} className="bg-[#2E8B57] hover:bg-[#246b47]">
              Create Account
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showTempPassword} onOpenChange={setShowTempPassword}>
        <DialogContent className="max-w-md">
          ...
          <div className="py-4 space-y-4">
            ...
            {[
              { label: 'Email', value: tempPasswordData?.email || '', mono: false },
              { label: 'Temporary Password', value: tempPasswordData?.temp_password || '', mono: true },
            ].map((f) => (
              <div key={f.label}>
                <Label className="text-xs text-gray-500">{f.label}</Label>
                <div className="flex items-center gap-2 mt-1">
                  <Input value={f.value} readOnly className={`flex-1 ${f.mono ? 'font-mono' : ''}`} />
                  <Button size="sm" variant="outline" onClick={() => copyToClipboard(f.value)}>
                    {copied ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                  </Button>
                </div>
              </div>
          ...
          <DialogFooter>
            <Button onClick={() => { setShowTempPassword(false); setTempPasswordData(null); }} className="bg-[#2E8B57] hover:bg-[#246b47] w-full">Done</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showDeactivateDialog} onOpenChange={setShowDeactivateDialog}>
        <DialogContent className="max-w-sm">
          ...
          <p className="py-4 text-sm">Are you sure you want to deactivate <strong>{deactivateTarget?.user_full_name}</strong>? They will no longer be able to access this portal.</p>
      ...
      <Dialog open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
        <DialogContent className="max-w-sm">
          ...
          <p className="py-4 text-sm">Are you sure you want to log out?</p>
          <DialogFooter className="gap-2">
